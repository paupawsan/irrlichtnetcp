...
IntPtr SceneManager_AddHillPlaneMesh(IntPtr scenemanager, M_STRING name, M_DIM2DF tileSize, M_DIM2DS tileCount, float hillHeight, M_DIM2DF countHills, M_DIM2DF textureRepeatCount)
{
   return GetSceneFromIntPtr(scenemanager)->addHillPlaneMesh(name, MU_DIM2DF(tileSize), MU_DIM2DS(tileCount), 0, hillHeight, MU_DIM2DF(countHills), MU_DIM2DF(textureRepeatCount));
}
...
void SceneManager_SetShadowColor(IntPtr scenemanager, M_SCOLOR color)
{
 ...
 void SceneManager_LoadScene(IntPtr scenemanager, M_STRING filename)
 {
